...
## 🎯 Новые возможности (Обновление 2025-08-09)

### ✅ Проверка прав пользователей в навигации

**Что добавлено:**
- Автоматическое скрытие пунктов меню на основе прав пользователя
- Динамическая навигация в зависимости от роли и разрешений
- Улучшенная безопасность и UX для пользователей с ограниченными правами

**Как это работает:**
1. **Администраторы** (`role: 'admin'`) видят все разделы админки
2. **Редакторы** (`role: 'editor'`) видят только те разделы, к которым у них есть права на чтение
3. Пункты меню автоматически скрываются, если у пользователя нет соответствующих прав

**Затронутые разделы:**
- Главная страница (`about_content.read`)
- Шапка сайта (`header_content.read`)
- Контакты (`contacts_content.read`)
- Резервные копии (`admin_backup.read`)
- Проекты (`projects.read`)
- Пользователи (`admin_users.read`)

**Технические детали:**
- Новая функция `canViewSection()` в `stores/admin.js`
- Обновленный компонент `AdminLayout.vue` с условным рендерингом
- Проверка прав на основе JSON структуры permissions пользователя
...
- **Компоненты админ-панели**
  ...
  - `AdminLayout.vue` - основной лайаут с навигацией (с проверкой прав)
...
- **Система состояния**
  - `stores/admin.js` - управление состоянием авторизации с проверкой прав
...
✅ **Новая функциональность** - автоматическое скрытие пунктов меню на основе прав пользователя

**Реализованные проверки:**
- **Главная страница** - требует права `about_content.read`
- **Шапка сайта** - требует права `header_content.read`
- **Контакты** - требует права `contacts_content.read`
- **Резервные копии** - требует права `admin_backup.read`
- **Проекты** - требует права `projects.read`
- **Пользователи** - требует права `admin_users.read`

**Логика работы:**
1. Пользователи с ролью `admin` видят все разделы
2. Пользователи с ролью `editor` видят только те разделы, к которым у них есть права на чтение
3. Если у пользователя нет прав на просмотр раздела, пункт меню не отображается

**Техническая реализация:**
- Функция `canViewSection()` в `stores/admin.js` проверяет права пользователя
- Компонент `AdminLayout.vue` использует `v-if="canViewSection('section_name')"` для условного отображения
- Проверка происходит на основе структуры прав пользователя в JSON формате
...
- **admin** - полные права на все разделы (всегда видит все пункты меню)
- **editor** - ограниченные права в зависимости от настроек (видит только разрешенные разделы)
